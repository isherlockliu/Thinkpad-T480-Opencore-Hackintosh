...
//    Warnings: Type mismatch between external declaration and actual object declaration detected
//    External (_PR_.PR00, DeviceObj)
    ...
    External (_SB_.PCI0.LPCB.EC__.XWAC, IntObj)
    ...
    External (_SB_.PCI0.RP01.DGIZ, UnknownObj)
...
//    Errors: Sytax error
//    One    
    OperationRegion (GNVS, SystemMemory, 0x5FF4F000, 0x0792)
...
//            Warnings: Device object requires either a _HID or _ADR, but not both          
//            Name (_ADR, 0x00)  // _ADR: Address
    ...
    Scope (\)
    {
        //        Warnings: Static OperationRegion should be declared outside control method
...
//            Warnings: Not all control paths return a value
            Else
            {
                Return (Zero)
            }
...
//                Warnings: Not all control paths return a value
        ...
        Device (RP09)
        {
            ...
            OperationRegion (PXCS, SystemMemory, 0xF80E8000, 0x0480)
            Field (PXCS, AnyAcc, NoLock, Preserve)
            {
                VDID,   32, 
                Offset (0x50), 
                    ...
                    If ((Arg0 == ToUUID ("f6c13ea5-65cd-461f-ab7a-29f7e8d5bd61") /* Unknown UUID */))
                    {
                        If ((Arg1 >= Zero))
                        {
                            Switch (ToInteger (Arg2))
                            {
                                ...
                                Case (0x09)
                                {
//                                    Warnings: Switch expression is not a static Integer/Buffer/String data type,defaulting to Integer
//                                    Switch (EMDS)
    ...
    Scope (\_SB.PCI0.LPCB)
    {
            ...
            Device (BAT1)
            {
                ...
                Method (_BTP, 1, NotSerialized)  // _BTP: Battery Trip Point
                {
                    ...
                    If (Arg0)
                    {
                        ...
                        Local1 = AJTP (0x10, Local1, Local0)
                        HT1L = (Local1 & 0xFF)
        ...
        Device (MOU)
        {
            Name (_HID, EisaId ("LEN0093"))  // _HID: Hardware ID
    ...
    Method (OWAK, 1, NotSerialized)
    {
        ...
        If ((Arg0 < 0x04))
        {
            If (((\RRBF & 0x02) || (\_SB.PCI0.LPCB.EC.XWAC & 0x02)))
            {
                Local0 = (Arg0 << 0x08)
...
//        OperationRegion (CMOS, SystemIO, 0x70, 0x04)
    ...
    Scope (\_GPE)
    {
        ...
        {
            Local0 = \_SB.PCI0.LPCB.EC.XWAC /* External reference */
            \RRBF = Local0
            Sleep (0x0A)
            If ((Local0 & 0x02)){}
            If ((Local0 & 0x04))
    ...
    OperationRegion (MNVS, SystemMemory, 0x5FF60018, 0x1000)
...
//                Name (_ADR, 0x00)  // _ADR: Address
    ...
    Scope (\_SB.PCI0.LPCB.EC)
    {
        ...
        {
            Fatal (0x01, 0x80010000, 0x00012007)
        }
...
//                                Switch (Local3)
                            ...
                            Case (0x0D)
                            {
                                ...
                                ElseIf (((Local3 == 0x02) || ((Local3 == 0x07) || (Local3 == 
                                    0x08))))
                                {
                                    \VPSC = 0x01
                                    \SPSC = Local3
        ...
        Method (DNCG, 0, Serialized)
        {
            If (((\_SB.PCI0.LPCB.EC.CHGP == 0x01) || (\VSTP == 0x01)))
            {
                DEVT (0xD5)
            }
            ElseIf ((\_SB.PCI0.LPCB.EC.AC._PSR () == 0x01))
            {
                DEVT (0xD1)
            }
            Else
        ...
        Method (DEVT, 1, Serialized)
        {
            If ((ANDN == One))
            {
                ...
                Notify (\_SB.PCI0.RP01.PEGP, Arg0)
                Notify (\_SB.PCI0.RP01.PXSX, Arg0)
                \_SB.PCI0.LPCB.EC.BVPC = One
            }
        }

        Method (UICF, 3, Serialized)
        {
            If ((0xFF != Arg0))
            {
                If (!CondRefOf (\_SB.IETM.DPTE))
...
//        Warnings: Unknown reserved name 
//        Method (_C2K, 1, Serialized)
        Method (C2K, 1, Serialized)
        {
